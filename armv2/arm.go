...
type xArm struct {
	...
	// The `shutdownCtx` lives from construction until resource `Close`.
	shutdownCtx context.Context
	shutdownFn  func()

	// requests lives from construction until close.
	requests chan request

	connMu sync.Mutex
	// `conn`, `responses` and `tidToResponseCh` each have the same lifetime. When a connection is
	// lost, we will never get responses for outstanding requests.
	conn            net.Conn
	responses       chan response
	tidToResponseCh map[uint16]chan response
...
func NewXArm(ctx context.Context, name resource.Name,
	...
	x.conn, err = (&net.Dialer{}).DialContext(ctx, "tcp", x.conf.host())
	if err != nil {
		return nil, err
	}
