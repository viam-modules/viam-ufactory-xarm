...
func (x *xArm) newCmd(reg byte) cmd {
	newVal := uint16(x.tid.Add(1) % (1 << 16))
	return cmd{tid: newVal, prot: 2, reg: reg}
}

func (x *xArm) writeLoop(ctx context.Context, conn net.Conn) {
	for {
		select {
		case <-ctx.Done():
			// We're being shutdown. We call `closeConn` to:
			// - Send errors back to any outstanding callers.
			// - Unstick any goroutines in a `conn.Read` method.
			//
			// In the case the connection died, this is a no-op.
			x.closeConn(conn)
			return
		case req := <-x.requests:
			x.tidToResponseCh[req.cmd.tid] = req.responseCh
			if err := writeBytesNew(x.shutdownCtx, conn, req.cmd); err != nil {
				x.closeConn(conn)
				return
			}
		case resp, ok := <-x.responses:
			if !ok {
				// `x.responses` being closed means `closeConn` was already called. Can just return
				// from here.
				return
			}

			respCh := x.tidToResponseCh[resp.cmd.tid]
			delete(x.tidToResponseCh, resp.cmd.tid)
			respCh <- resp
			close(respCh)
		}
	}
...
func (x *xArm) readLoop(ctx context.Context, conn net.Conn) {
	for {
		if ctx.Err() != nil {
			return
		}

		respBytes, err := readResponseBytes(conn)
		if err != nil {
			x.closeConn(conn)
		}
	}
...
func (x *xArm) closeConn(conn net.Conn) {
	x.connMu.Lock()
	defer x.connMu.Unlock()

	if x.conn != conn {
		// Someone else won the race, nothing to do.
		return
	}

	conn.Close()
	x.connCancel()
	for tid, respCh := range x.tidToResponseCh {
		respCh <- response{cmd{}, errors.New("connection reset")}
		close(respCh)
	}
	x.tidToResponseCh = make(map[uint16]chan response)
	close(x.responses)
	x.conn = nil
}
...
func (x *xArm) send(ctx context.Context, c cmd, checkError bool) (cmd, error) {
	...
	x.requests <- request{
		request: c, responseCh: responseCh,
	}

	// We always wait for a response, whether `checkError` is true or not.
	var resp cmd
	select {
	case <-ctx.Done():
		return cmd{}, ctx.Err()
	case respObj, ok := <-responseCh:
		if !ok {
			return cmd{}, errors.New("Response not received")
		}

		if respObj.err != nil {
			return cmd{}, respObj.err
		}
	...
	//
	// Dan: What cases are warnings generated? Do we want to return an error on a warning? I would
	// assume a warning means the operation was successfully completed. So it's not clear to me that
	// the application code should be told there was a problem.
	if state&(1<<6|1<<5) == 0 {
		return resp, nil
	}
...
func writeBytesNew(ctx context.Context, conn net.Conn, cmd cmd) error {
	buf := cmd.bytes()

	totalWrote := 0
	for totalWrote < len(buf) {
		wrote, err := conn.Write(buf)
		if err != nil {
			return err
		}

		if totalWrote == 0 && wrote < len(buf) {
			panic(fmt.Sprintf("old code made bad assumption. wrote:", wrote, "to write:", len(buf)))
		}
...
func commandFromBytes(buf []byte) cmd {
	return cmd{
		tid:    binary.BigEndian.Uint16(buf[0:2]),
		prot:   binary.BigEndian.Uint16(buf[2:4]),
		reg:    buf[6],
		params: buf[7:],
	}
}
...
func readResponseBytes(conn net.Conn) ([]byte, error) {
	...
	paramsLength := int(binary.BigEndian.Uint16(header[4:6]) - 1)
	ret := make([]byte, len(header)+paramsLength)
	copy(ret, header)
	_, err = io.ReadFull(conn, ret[len(header):])
	if err != nil {
		return nil, err
	}
...
func (x *xArm) start(ctx context.Context, conn net.Conn) error {
	go x.writeLoop(x.shutdownCtx, conn)
	go x.readLoop(x.shutdownCtx, conn)
